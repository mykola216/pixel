...
class WC_Integration_Brandbassador_Integration extends WC_Integration {
	...
	public function init_form_fields() {
		$this->form_fields = array(
            ...
            'api_key_back' => array(
                'title'             => __( 'API Key Back', 'woocommerce-integration-brandbassador' ),
                'type'              => 'text',
                'description'       => __( 'API Key Back', 'woocommerce-integration-brandbassador' ),
                'desc_tip'          => true,
                'default'           => ''

            ),
			'debug' => array(
				'title'             => __( 'Debug Log', 'woocommerce-integration-brandbassador' ),
				'type'              => 'checkbox',
				'label'             => __( 'Enable/Exclude', 'woocommerce-integration-brandbassador' ),
				'default'           => 'no',
				'description'       => __( 'Enable/Exclude display of pixels', 'woocommerce-integration-brandbassador' ),
			),
		);
	}
    ...
    // Checking from admin ********** [-_-] **********
    function fields_debug_Checking() {
        $fields_debug_Checking = false;
        if ($this->get_option( 'debug') == 'yes') {
            $fields_debug_Checking = true;
        }
        return $fields_debug_Checking;
    }
}
...
/*****************************************************/
    ...
    if (count($url_page_cupon) > 3) {
        if (isset($url_page_cupon) and $url_page_cupon[2] == 'order-received') {
            ...
            $fields_debug_Checking = $WC_Integration_Brandbassador_Integration->{'fields_debug_Checking'}(); // debug
            ...
            function isa_order_received_text()
            {
                ...
                $WC_Integration_Brandbassador_Integration = new WC_Integration_Brandbassador_Integration;
                $fields_debug_Checking = $WC_Integration_Brandbassador_Integration->{'fields_debug_Checking'}(); // debug
                if ($fields_debug_Checking) {
                    //if ($fields_debug_Checking and !$fields_api_key_back_Checking == '') {
